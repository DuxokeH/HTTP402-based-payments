// X402 showcase — three tabs. Enkratno = MetaMask (viem); tx & merjeno = SSE M2M.
import {
...
async function payEnkratno(demo) {
  ...
  try {
    ...
    setStep('e-s3', 'active');
    s = now();
    const vr = await fetch('/enkratno/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestId: payment.requestId, txHash, network: payment.network, payerAddress: payer }) });
    const vj = await vr.json();
    if (!vr.ok) { setStep('e-s3', 'fail'); throw new Error(vj.message || vj.error || 'Preverjanje ni uspelo'); }
    T.preverjanje = now() - s; $('e-t3').textContent = ms(T.preverjanje); setStep('e-s3', 'done');

    // 4 — access
    setStep('e-s4', 'active');
    s = now();
    const ar = await fetch('/enkratno/service', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Payment': vj.proofToken }, body: JSON.stringify({ prompt }) });
