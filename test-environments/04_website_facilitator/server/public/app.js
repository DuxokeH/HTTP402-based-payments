// X402 showcase — FACILITATOR BRANCH (folder 04_website_facilitator).
// Three tabs. Single = MetaMask (viem); tx & metered = SSE M2M.
// Difference from folder 05: step 3 (the payment report) goes DIRECTLY to the
// facilitator, not to the merchant — see the facilitator protocol description
// in ../README.md.
import {
...
async function payOneTime(demo) {
  ...
  try {
    ...
    // 3 — payment report to the FACILITATOR (arrow C→F)
    // This is where the facilitator branch diverges from the direct one: the
    // payment is not reported to the merchant but straight to the facilitator,
    // the only party that reads the chain and issues the proof token.
    setStep('e-s3', 'active');
    s = now();
