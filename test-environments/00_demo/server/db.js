...
const DB_PATH = process.env.DB_PATH || path.join(DATA_DIR, 'x402.db');
...
const stmts = {
  insertRequest: db.prepare(`
    INSERT INTO payment_requests
      (request_id, recipient, amount_eth, currency, network, payer_address, created_at, expires_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `),
  getRequest: db.prepare('SELECT * FROM payment_requests WHERE request_id = ?'),
  insertProof: db.prepare(`
    INSERT INTO payment_proofs
      (proof_token, request_id, tx_hash, block_number, payer_address, recipient, amount_eth, verified_at, expires_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `),
...
function createPaymentRequest({ requestId, recipient, amountEth, currency, network, payerAddress, ttlSeconds }) {
  const now = Date.now();
...
function createProof({ proofToken, requestId, txHash, blockNumber, payerAddress, recipient, amountEth, ttlSeconds }) {
  const now = Date.now();
...
function markTxRedeemed(txHash, requestId) {
...
function healthCheck() {
  try {
    return stmts.healthCheck.get().ok === 1;
  } catch {
    return false;
  }
}
