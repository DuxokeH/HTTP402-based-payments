...
def find(mapa, imena):
    d = os.path.join(ROOT, mapa, "measurements")
    for n in imena:
        p = os.path.join(d, n)
        if os.path.exists(p):
            return p
...
def main():
    ...
    if args.vzorec or "_sample" in str(tx_csv) or "_sample" in str(db_csv):
        mark_sample(True)
    ...
    modelirano = tx["fee_eth"].notna().sum() == 0
    if modelirano:
        fee_tx = args.gas_per_tx * args.gas_price_gwei / 1e9
        fee_topup = fee_tx
        oznaka = f"modelled @ {args.gas_price_gwei:g} gwei"
        oznaka_sl = f"modelirano @ {args.gas_price_gwei:g} gwei"
    else:
    ...
    ax.plot(N, strosek_tx, color=ORANGE, linewidth=2.4, label=t("1 transaction / query (folder 02)", "1 transakcija / poizvedbo (mapa 02)"))
    ax.plot(N, strosek_seja, color=BLUE, linewidth=2.4, label=t("metered session: 1 top-up (folder 03)", "merjena seja: 1 polnitev (mapa 03)"))
    ...
    ax.set_yscale("log"); ax.set_ylabel(t("time [ms] (logarithmic scale)", "čas [ms] (logaritemska skala)"))
    ...
    tab = pd.DataFrame(vrst, columns=cols_en)
    ...
    ax.set_title(t(f"Settlements and cost at N uses ({oznaka})", f"Poravnave in strošek pri N uporabah ({oznaka_sl})"), fontsize=11, fontweight="bold", pad=12)
