...
# -*- coding: utf-8 -*-
"""
Latency analysis of the MERGED one-time payment (folder 06) — 4 HTTP messages.
...
Usage:
  ...
  python3 latency_analysis.py ../measurements/merged_real.csv
  python3 latency_analysis.py ../measurements/merged_mock.csv --out figures
"""
...
def find_csv():
    ...
    for name in ("merged_real.csv", "merged_mock.csv", "_sample/merged_real.csv", "_sample/merged_mock.csv"):
...
def main():
    ...
    PHASES = [("t_challenge_ms", t("402 challenge", "Izziv 402")),
              ...
              ("t_merged_ms", t("Merged: proof + delivery", "Združeno: dokaz + dostava"))]
    ...
    ax.set_title(t(f"Merged payment latency (4 messages) by phase  ·  mode: {mode}  (n={len(df)})",
                   f"Latenca združenega plačila (4 sporočila) po fazah  ·  način: {mode}  (n={len(df)})"))
    clean_axes(ax)
    ...
    colors = [BLUE, ORANGE, AQUA, "#eda100"][:len(active)]
    ...
    ax.set_title(t(f"Median phase breakdown of the merged flow  (total ≈ {sum(med):.0f} ms)",
                   f"Medianska sestava združenega poteka po fazah  (skupaj ≈ {sum(med):.0f} ms)"))
