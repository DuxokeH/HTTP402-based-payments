...
# -*- coding: utf-8 -*-
"""
Analiza latence ZDRUŽENEGA enkratnega plačila (mapa 06) — 4 sporočila HTTP.
...
Uporaba:
  ...
  python3 analiza_latence.py ../meritve/zdruzena_real.csv
  python3 analiza_latence.py ../meritve/zdruzena_mock.csv --out slike
"""
...
FAZE = [("t_izziv_ms", "Izziv 402"), ("t_oddaja_ms", "Oddaja tx"),
        ("t_potrditev_ms", "Potrditev bloka"), ("t_zdruzeno_ms", "Združeno: dokaz + dostava")]

def poisci_csv():
    d = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "meritve")
    for name in ("zdruzena_real.csv", "zdruzena_mock.csv", "_vzorec/zdruzena_real.csv", "_vzorec/zdruzena_mock.csv"):
...
def main():
    ...
    ax.set_title(f"Latenca združenega plačila (4 sporočila) po fazah  ·  način: {nacin}  (n={len(df)})")
    ocisti_osi(ax)
    ...
    barve = [MODRA, ORANZNA, AKVA, "#eda100"][:len(aktivne)]
    ...
    ax.set_xlabel("čas [ms]"); ax.set_title(f"Medianska sestava združenega poteka po fazah  (skupaj ≈ {sum(med):.0f} ms)")
